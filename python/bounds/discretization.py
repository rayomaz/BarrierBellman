...
class BoundButcherTableau(BoundModule):
    ...
    def subnetwork_backward_relaxation(self, network, relaxation_step, region):
        linear_bounds, relaxation_module, *extra = relaxation_step.backward_relaxation(region)
        ...
        for i, bound_update in reversed(list(enumerate(network))):
            ...
            bounds = bound_update.crown_backward(dt_bounds, False)
        ...
        return LinearBounds(linear_bounds.region, lower, upper), relaxation_module, *extra
    ...
    def bound_parameters(self):
        for module in self.bound_update:
